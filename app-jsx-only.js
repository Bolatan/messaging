// Save this file as: client/src/App.jsx
...
const WhatsAppClone = () => {
  ...
  useEffect(() => {
    ...
    socketRef.current.on('typing:display', () => setIsTyping(true));
    socketRef.current.on('typing:hide', () => setIsTyping(false));
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-4 bg-teal-50">
              {messages.map(msg => {
                ...
                return (
                  <div key={msg._id} className={`flex mb-4 ${isSent ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-lg ${
                      isSent ? 'bg-teal-500 text-white rounded-br-none' : 'bg-white text-gray-800 rounded-bl-none'
                    } shadow`}>
                      {selectedChat.isGroup && !isSent && (
                        <p className="text-xs font-semibold text-teal-600 mb-1">{msg.senderId.name}</p>
                      )}
            ...
            <div className="bg-gray-50 border-t border-gray-200 p-4">
              <div className="flex items-center gap-2">
                ...
                />
                {message.trim() ? (
                  <button onClick={handleSendMessage} className="p-2 bg-teal-500 hover:bg-teal-600 rounded-full">
